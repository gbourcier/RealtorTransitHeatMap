...
import { listListings, type Listing, type SortBy, type SortDir } from '../api/listings'
...
async function load() {
  ...
  try {
    ...
    error.value = e?.response?.data?.error ?? e?.message ?? 'failed to load listings'
  } finally {
    loading.value = false
  }
}
        ...
        <thead>
          <tr>
            ...
            <th class="sortable-col text-right" @click="toggleSort('price')">
              Price
              <v-icon size="small" class="sort-icon">{{ sortIcon('price') }}</v-icon>
            </th>
            <th class="sortable-col" @click="toggleSort('first_seen_at')">
              First Seen
              <v-icon size="small" class="sort-icon">{{ sortIcon('first_seen_at') }}</v-icon>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="`${item.board}-${item.mls}`">
            ...
            <td>{{ item.address || '—' }}</td>
            <td class="text-right">{{ formatPrice(item.currentPrice) }}</td>
